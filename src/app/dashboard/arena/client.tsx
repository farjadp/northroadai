...
export default function ArenaClient() {
  ...
  const finishFight = async () => {
    ...
    try {
        const res = await fetch("/api/arena/report", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                        ...
                        <div className="space-y-2 overflow-y-auto h-full pb-20">
                            ...
                            {pastBattles.map(battle => (
                                <div 
                                    ...
                                    className={`p-4 border rounded-xl cursor-pointer transition-all hover:bg-white/5 ${
                                        sessionId === battle.id ? "border-white bg-white/10" : "border-white/10 bg-black"
                                    }`}
                                >
